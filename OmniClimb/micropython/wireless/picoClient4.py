...
# --- NEW CONSTANT: Desired sampling interval ---
_SAMPLE_INTERVAL_MS = const(20) # Desired interval between consecutive samples (e.g., 20ms for 50Hz)

# A single frame buffer to hold data before pushing to queue.
# Using 'i' for signed int to accommodate larger relative timestamps if needed, or future absolute timestamps
frame_buffer_raw = array('l', (0 for _ in range(_SAMPLES_PER_FRAME * _CHANNELS_PER_SAMPLE)))
...
def core0_record_adc_data_frame():
    ...
    with lock: # Acquire lock to check recording_active before appending
        if recording_active: # Only append the frame if recording is still desired
            data_queue.append(array('i', frame_buffer_raw)) # Append a *copy* of the completed frame
            ...
            # this full frame is discarded. The next call to this function will stop immediately.
...
def main_loop():
    ...
    try:
        ...
        # Mount SD card on startup, it will remain mounted unless error or script exit
        ...
        while True: # This loop keeps the Pico running and ready for commands
            try:
                ...
                utime.sleep_ms(1) # Yield control frequently
                ...
                if utime.time() - last_status_publish_time > status_publish_interval:
                    with lock: # Protect access to recording_active
                        ...
                        elif sd_card_present: # Check global sd_card_present status
                ...
                # Reconnection logic handles most network errors, allows loop to continue
                if reconnect_attempts < max_reconnect_attempts:
                    ...
                    try:
                        if not wlan_local_ref.isconnected():
                            ...
                            if not wlan_local_ref.isconnected():
                                ...
                                # If Wi-Fi consistently fails, it's a critical error for connectivity
                                raise RuntimeError("WiFi reconnect failed") # This will break the main loop
                    ...
                    break # Critical failure, exit main loop

            except Exception as e:
                print(f"Unexpected error in main loop: {e}")
                sys.print_exception(e)
                print("Exiting main loop due to unexpected error.")
                break # Catch all for unexpected critical errors

    except RuntimeError as e: # Catches initial setup failures like WiFi connect fail
        print(f"Initial setup failed: {e}")
        sys.print_exception(e)
    except KeyboardInterrupt: # Allows user to stop script cleanly with Ctrl+C
        print("\nScript terminated by user (Ctrl+C)")
    finally:
        # This block now ONLY executes when the entire script is exiting (e.g., due to errors that break the loop, or KeyboardInterrupt)
        ...
        utime.sleep(3) # Ensure writer thread has time to finish

        if sd_card_present: # Only unmount if it was mounted
            try:
                uos.umount("/sd")
        ...
        if client: # Only disconnect if client object exists
            try:
                client.disconnect()
