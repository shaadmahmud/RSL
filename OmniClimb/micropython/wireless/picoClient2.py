...
def connect_to_network():
    ...
    max_wait = 60 # Increased wait time for robust connection
...
def core0_record_adc_data_frame():
    global recording_active, led, lock, sd_card_present, data_queue, frame_buffer_raw

    if not recording_active: # Only sample if recording is active
        return

    # Sample each channel in single-shot mode with a timestamp
    t_frame_start = ticks_ms()
    
    for sample_idx in range(_SAMPLES_PER_FRAME):
        timestamp = ticks_diff(ticks_ms(), t_frame_start) # Timestamp relative to frame start
        
        # Calculate start index for current sample in the 1D array
        base_idx = sample_idx * _CHANNELS_PER_SAMPLE
        
        # Store timestamp
        frame_buffer_raw[base_idx + 0] = timestamp
        
        # Read ADS1115 channels (assuming A0-A3)
        # Note: ads.read() returns 16-bit integer
        frame_buffer_raw[base_idx + 1] = ads.read(channel1=0) # Vref_val
        frame_buffer_raw[base_idx + 2] = ads.read(channel1=1) # Vout_a301_val
        frame_buffer_raw[base_idx + 3] = ads.read(channel1=2) # Vout_a401_val
        frame_buffer_raw[base_idx + 4] = ads.read(channel1=3) # Vout_empty_val

        # A short sleep to yield control within a high-frequency loop, if necessary.
        # However, ads.read() takes time, so this might not be needed.
        # utime.sleep_us(100) # Adjust based on your sampling rate goals

    # After filling frame_buffer_raw, transfer it to data_queue for Core 1 to write
    with lock:
        # Append a *copy* of the array.array object to the queue.
        # This is important so core1_write2sd gets a snapshot.
        data_queue.append(array('h', frame_buffer_raw))

    # Indicate recording on LED (continuously on while core0 is sampling)
    led.value(1)
...
def start_adc_recording(filename_from_cmd):
    ...
    with lock:
        ...
        # LED will be turned on by core0_record_adc_data_frame
    ...
    try:
        ...
        while True:
            try:
                ...
                with lock: # Protect recording_active check
                    if recording_active:
                        # Call the ADC sampling function which runs on Core 0
                        # This function will fill a frame buffer and append it to data_queue
                        ...
                        # For 100Hz (10ms interval per sample), and
                        # this frame takes ~500ms to fill.
                        # So, you don't need a sleep here in the loop if sampling is continuous.
                        # If you want a pause *between* frames, add utime.sleep_ms() here.
                        # Let's add a small sleep to ensure cooperative multitasking.
                ...
                # Small delay to yield CPU, but keep checking msg often
                # If ADC sampling rate is slow, this can be higher. If fast, lower.
                # Since core0_record_adc_data_frame samples a frame at a time,
                # the overall loop iteration will be dominated by that.
                # utime.sleep_ms(10) # Removed, as sampling loop is now main loop activity
        ...
        utime.sleep(2) # Give some time for Core 1 to clear its queue
